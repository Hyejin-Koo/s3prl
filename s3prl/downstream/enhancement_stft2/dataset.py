...
class SeparationDataset(Dataset):
    def __init__(
        ...
        for cond in src + tgt:
            ...
            try:
                os.path.exists("{}/{}/wav.scp".format(self.data_dir, cond))
            except:
            ...
            for line in content:
                ...
                if cond == "clean" and "AIhub" in self.data_dir : #and 'test' not in self.data_dir: ## bad..
                    if '/data/noisy_AIhub' not in path and 'korean' not in path:
                        ...
                        path = os.path.join(postdir,"_".join(subdir.split('_')[:-1]),uttname+'.wav')
                    else:
                        directory, filename = os.path.split(path)
                        path = os.path.join(directory,"_".join(uttname.split('_')[:-1]),uttname+'.wav')            
               
                #elif 'test' in self.data_dir:
                #    directory, filename = os.path.split(path)
                #    path = os.path.join(directory,"_".join(uttname.split('_')[:-1]),uttname+'.wav')
                else:
                    path = os.path.join("/".join(path.split('/')[:-1]),uttname+'.wav')
...
class SeparationInferenceDataset(Dataset):
    def __init__(
        self,
        data_dir,
        rate=16000,
        ...
        super(SeparationInferenceDataset, self).__init__()
        """
        Args:
            data_dir (str):
                prepared data directory
...
#        assert len(self.src) == 1 and len(self.tgt) == 1
        ...
        #cond_list = ["noisy", "clean"]
        cond_list = ["noisy"] #inferece dataset is not separated by noisy or clean
        cond_list = ["clean"] #inference test about clean dataset 
        # create the mapping from utterances to the audio paths
#        # reco2path[utt][cond] is the path for utterance utt with condition cond
#        reco2path = {}
#        for cond in src :#+ tgt:
...
#                    if '/data/noisy_AIhub' not in path:
#                        directory, filename = os.path.split(path)
#                        postdir, subdir = os.path.split(directory)
#                        path = os.path.join(postdir,"_".join(subdir.split('_')[:-1]),uttname+'.wav')
#                    else:
...
#        self.recolist = list(self.reco2path.keys())
#        self.recolist.sort()
        self.recolist = [i for i in os.listdir(self.data_dir) if '.wav' in i]
        for wav in self.recolist:
            self.recolist[self.recolist.index(wav)]=os.path.join(self.data_dir,wav)
...
#
        ...
        wav_length (tensor):
            number of samples in each utterance
        """
        return source_wav_list, uttname_list, source_attr, source_wav, feat_length, wav_length
